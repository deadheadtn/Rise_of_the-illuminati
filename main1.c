...
void mvt_clavier (SDLKey bouton, SDL_Rect *bg, SDL_Surface *imageDeFondCollision, SDL_Rect *positionpers, int *mouvement, SDL_Rect *pospers, SDL_Surface **image, int *vit1, int *vit2, int *vit3, int *vit4, int *ok)
=======
    f=fopen(chemin,"w");
    if(f == NULL)
        return(-1);
     ...
     SDL_PollEvent(&event);
        switch (event.type)
        {
            ...
            case SDL_QUIT : 
             ...
             case SDL_KEYDOWN :
             {
              mvt_clavier (event.key.keysym.sym, &bg,imageDeFondCollision,&positionpers,&mouvement,&pospers, &image, &vit1, &vit2, &vit3, &vit4,&ok);
